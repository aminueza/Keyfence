...
character, and the entry carries the kind and the length instead: four
characters at each end of a short password is most of the password.
`key` is the JSON key the
