...
def test_render_summarises(home):
    ...
    assert "Everything looks good." in doctor.render([doctor.Check(doctor.OK, "a", "")])
...
def test_ca_checks(tmp_path, monkeypatch):
    ...
    assert doctor.check_ca(missing).status == doctor.WARN
    ...
    assert doctor.check_ca_trusted(missing, run=lambda cmd: (44, "")).status == doctor.WARN
    assert doctor.check_ca_trusted(tmp_path / "none.pem", run=lambda cmd: (0, "")).status == doctor.WARN
    monkeypatch.setattr(doctor.platform, "system", lambda: "Linux")
    assert doctor.check_ca_trusted(missing).status == doctor.WARN
...
def test_proxy_and_environment_checks(monkeypatch, tmp_path):
    ...
    ca = tmp_path / "ca.pem"
    assert doctor.check_environment(8888, {}, ca).status == doctor.WARN
...
def test_local_mode_check(monkeypatch):
    monkeypatch.setattr(doctor.platform, "system", lambda: "Linux")
    assert doctor.check_local_mode().status == doctor.WARN
    monkeypatch.setattr(doctor.platform, "system", lambda: "Windows")
    assert doctor.check_local_mode().status == doctor.WARN
    monkeypatch.setattr(doctor.platform, "system", lambda: "Darwin")
    assert doctor.check_local_mode(run=lambda cmd: (1, "")).status == doctor.WARN
...
def test_hook_check(home, tmp_path, monkeypatch):
    ...
    assert doctor.check_hook(tmp_path / "proj").status == doctor.WARN
...
def test_audit_check(home, write_config):
    assert doctor.check_audit().status == doctor.WARN
